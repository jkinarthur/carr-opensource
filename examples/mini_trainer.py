...
if __name__ == "__main__":
    ...
    model = MiniBackbone(num_items=n_items, d_model=d_model, n_layers=n_layers).to(device)
    selector = GumbelLayerSelector(num_layers=n_layers, init_temperature=2.0)
    adaptive_weights = AdaptiveLossWeights()
    loss_fn = UnifiedCompressionLoss(UnifiedLossConfig())

    inner_params = list(model.parameters()) + list(selector.parameters())
    inner_optimizer = torch.optim.Adam(inner_params, lr=lr_model)
    lambda_optimizer = torch.optim.Adam(adaptive_weights.parameters(), lr=lr_lambda)
    bilevel = BilevelLambdaOptimizer(inner_optimizer=inner_optimizer, lambda_optimizer=lambda_optimizer)
    convergence_log = CriticalDepthConvergenceLogger()

    depth_csv = open(os.path.join(out_dir, "mini_trainer_depth_convergence.csv"), "w", newline="", encoding="utf-8")
    lambda_csv = open(os.path.join(out_dir, "mini_trainer_lambdas.csv"), "w", newline="", encoding="utf-8")
    depth_writer = csv.writer(depth_csv)
    lambda_writer = csv.writer(lambda_csv)
    depth_writer.writerow(["epoch", "soft_depth", "hard_depth", "critical_depth", "abs_gap"])
    lambda_writer.writerow(["epoch", "lam_geometry", "lam_evidence", "lam_efficiency", "lam_entropy", "train_loss", "val_loss"])

    layer_weights = torch.linspace(1.0, 2.0, n_layers)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        seqs_t, targets_t = next(train_iter)
        seqs_t, targets_t = seqs_t.to(device), targets_t.to(device)

        def train_closure(lmb: dict) -> dict:
            logits, hiddens = model(seqs_t)
            rec_loss = F.cross_entropy(logits, targets_t)

            r_layers = torch.tensor([_compute_collapse_score(h)[0] for h in hiddens], dtype=torch.float32)
            evidence_layers = torch.tensor([_compute_collapse_score(h)[1] for h in hiddens], dtype=torch.float32)

            selection = selector(training=True)
            kappa = selection.soft_index
            r_pre = soft_precompression_reference(r_layers, kappa=kappa)
            post_mask = soft_postcompression_weights(num_layers=n_layers, kappa=kappa)
            m_star = torch.tensor(16.0)
        ...
        seqs_v, targets_v = next(val_iter)
        seqs_v, targets_v = seqs_v.to(device), targets_v.to(device)

        def val_closure(lmb: dict) -> dict:
            model.eval()
            with torch.no_grad():
                logits_v, hiddens_v = model(seqs_v)
            rec_loss_v = F.cross_entropy(logits_v, targets_v)

            r_layers_v = torch.tensor([_compute_collapse_score(h)[0] for h in hiddens_v], dtype=torch.float32)
            evidence_layers_v = torch.tensor([_compute_collapse_score(h)[1] for h in hiddens_v], dtype=torch.float32)

            selection_v = selector(training=False)
            kappa_v = selection_v.soft_index
            r_pre_v = soft_precompression_reference(r_layers_v, kappa=kappa_v)
            post_mask_v = soft_postcompression_weights(num_layers=n_layers, kappa=kappa_v)

            return loss_fn(
                rec_loss=rec_loss_v,
                r_layers=r_layers_v,
                r_pre_compress=r_pre_v,
                post_mask=post_mask_v,
                layer_weights=layer_weights,
                evidence_layers=evidence_layers_v,
                kappa=kappa_v,
                m_star=torch.tensor(16.0),
                selector_entropy=selection_v.entropy,
                num_layers=n_layers,
                seq_len=seq_len,
                lambda_overrides=lmb,
            )

        result = bilevel.step(train_closure, val_closure, lambda_provider=adaptive_weights)
        ...
        convergence_log.update(
            epoch=epoch, soft_depth=soft_d, hard_depth=hard_d, critical_depth=critical_depth_proxy
        )
        depth_writer.writerow([epoch, f"{soft_d:.6f}", hard_d, f"{critical_depth_proxy:.6f}", f"{abs(soft_d-critical_depth_proxy):.6f}"])
        lambda_writer.writerow([
            epoch,
            f"{result.lambda_geometry:.6f}", f"{result.lambda_evidence:.6f}",
            f"{result.lambda_efficiency:.6f}", f"{result.lambda_entropy:.6f}",
            f"{result.train_total:.6f}", f"{result.val_total:.6f}",
        ])

        if epoch % 10 == 0 or epoch == 1:
            print(
                f"Epoch {epoch:>3} | train={result.train_total:.4f} val={result.val_total:.4f} "
                f"| kappa={soft_d:.2f} (hard={hard_d}) "
                f"| λ=({result.lambda_geometry:.3f}, {result.lambda_evidence:.3f}, {result.lambda_efficiency:.3f}, {result.lambda_entropy:.3f})"
            )

    depth_csv.close()
    lambda_csv.close()
    print(f"\nConvergence stabilized@0.5: {convergence_log.stabilized(window=10, tol=0.5)}")
    print(f"Mean |kappa-k*| last 10 epochs: {convergence_log.mean_gap_last(10):.3f}")
